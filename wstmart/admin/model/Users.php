...
class Users extends Base{
	...
	public function pageQuery(){
		...
		/********************* 取数据 *************************/
		$rs = $this->where($where)
					->field(['userId','loginName','userName','userPhone','userEmail','userScore','userMoney','createTime','userStatus','lastTime'])
					->order('userId desc')
	...
	*/
	...
	public function recharge(){
		...
		$rs = db('users')->where('userId',$userId)->setInc('userMoney', $money);	
		// $rs = true;	
		//创建充值记录
		if($rs){
			$userName = db('users')->where('userId',$userId)->value('loginName');
			$staff = session('WST_STAFF');
			$lm = [];
			$lm['targetType'] = 0;
			$lm['targetId'] = $userId;
			$lm['dataSrc'] = 6;
			$lm['remark'] = $staff['roleName'].'【'.$staff['staffName'].'】向会员账号'.$userName.'充值¥'.$money;
			$lm['moneyType'] = 1;
			$lm['money'] = $money;
			$lm['payType'] = $payType;
