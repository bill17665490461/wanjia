...
class Shops extends Base{
	...
	public function pageQuery($shopStatus=1){
		return Db::table('__SHOPS__')->alias('s')->join('__AREAS__ a2','s.areaId=a2.areaId','left')
		       ...
		       ->field('shopId,shopSn,shopName,a2.areaName,shopkeeper,telephone,shopAddress,shopCompany,shopAtive,shopStatus,shopMoney')
   ...
   /* public function toAdopts(){
        ...
        try{
           // $result = $this->update($data,['shopId'=>$id]);
            $result = $this->where('shopId',$id)->setField('shopStatus',1);
	...
	public function edit(){
        ...
        if(!$validate->check(Input('post.')))return WSTReturn($validate->getError());
        //判断经营范围
        $goodsCatIds = input('post.goodsCatIds1').','.input('post.goodsCatIds2').','.input('post.goodsCatIds3');
        $accredIds = input('post.accredIds');
        if($goodsCatIds=='')return WSTReturn('请选择经营范围');
        Db::startTrans();
        try{
            ...
            Db::table('wst_shops')->where(['shopId'=>$shopId])->setField('CatIds', $goodsCatIds);
		    //认证类型
	...
	*/
	...
	public function getAgencyList(){
		$res = Db::name('users')->alias('us')->field('ag.agencyId,ag.realName')->join('__AGENCYS__ ag','us.userID = ag.userId')->where('userType=1 and us.dataFlag=1 and ag.dataFlag=1')->select();
		$data = array();
		foreach ($res as $k => $v) {
			$data[$v['agencyId']] = $v['realName'];
		}
		return $data;
	}
	...
	public function recharge(){
		...
		$rs = db('shops')->where('shopId',$shopId)->setInc('shopMoney', $money);	
		// $rs = true;	
		//创建充值记录
		if($rs){
			$shopName = db('shops')->where('shopId',$shopId)->value('shopName');
			$staff = session('WST_STAFF');
			$lm = [];
			$lm['targetType'] = 1;
			$lm['targetId'] = $shopId;
			$lm['dataSrc'] = 6;
			$lm['remark'] = $staff['roleName'].'【'.$staff['staffName'].'】向店铺【'.$shopName.'】充值¥'.$money;
			$lm['moneyType'] = 1;
			$lm['money'] = $money;
			$lm['payType'] = $payType;
