...
class CashDraws extends Base{
	...
	public function handle(){
		$id = (int)input('cashId');
		$targetType = (int)input('targetType');
		if($targetType==1){$mName='users';}elseif($targetType==2){$mName='shops';}elseif($targetType==3){$mName='agencys';}
		$cash = $this->get($id);
		if(empty($cash))return WSTReturn('无效的提现申请记录');
		$user = model($mName)->get($cash->targetId);
		...
		try{
            ...
            if(false != $result){
				...
				//发送信息信息
				WSTSendMsg($user['userId'],"您的提现申请单【".$cash->cashNo."】已通过，请留意您的账户信息",['from'=>5,'dataId'=>$id]);
				Db::commit();
