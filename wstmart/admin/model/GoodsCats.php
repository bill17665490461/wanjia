...
class GoodsCats extends Base{
    ...
    public function getListCats($str){
       $array = explode('_',$str);
        $result = array();
        if (is_array($array)){
            $result['topLevel'] = $this->where(['dataFlag'=>1,'parentId'=>0])->order('catSort asc,catName asc')->select();

            $result['interMediate'] = $this->where(['dataFlag'=>1,'parentId'=>$array[0]])->order('catSort asc,catName asc')->select();
            if (isset($array[1])) {
                $result['LastStage'] = $this->where(['dataFlag' => 1, 'parentId' => $array[1]])->order('catSort asc,catName asc')->select();
            }else{
                $result['LastStage'] = false;
            }
        }
        return $result;
    }
