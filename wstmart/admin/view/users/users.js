        ...
        columns: [
          ...
          { display: '余额', name: 'userMoney', isSort: false},
	        ...
	        { display: '操作', name: 'op',isSort: false,render: function (rowdata, rowindex, value){
              ...
              if(WST.GRANT.HYGL_04)h += "<a href='javascript:getForRecharge(" + rowdata['userId'] + ")'>充值</a> "; 
	            return h;
	        }}
        ]
    });
...
function getForRecharge(id){
    ...
    $.post(WST.U('admin/users/get'),{id:id},function(data,textStatus){
      ...
      if(json.userId){
        ...
        $('#account').html(json.loginName);
        toRecharge(json.userId);
      }else{
        WST.msg(json.msg,{icon:2});
      }
    });
...
function toRecharge(id){
    var box = WST.open({title:'编辑',type:1,content:$('#rechargeBox'),area: ['400px', '200px'],btn:['确定','取消'],yes:function(){
        ...
        var loading = WST.msg('正在提交数据，请稍后...', {icon: 16,time:60000});
        $.post(WST.U('admin/users/recharge'), params, function(data,textStatus){
