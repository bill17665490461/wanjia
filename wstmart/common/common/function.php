...
function WSTUploadPic($fromType=0){
    ...
    $validate = new \think\Validate([
	    ...
	    ['fileSize','fileSize:8388608','文件大小超出限制'],//最大2M
	]);
	$data = ['fileMime'  => $file,
	    	 'fileSize' => $file,
	         'fileExt'=> $file
 ...
 * 手机端压缩上传图片
 */
function mobUploadPic(){
	$dir = Input('param.dir');
	$uploadPic = input('post.uploadPic');
	$path = ROOT_PATH.'/upload/'.$dir."/".date('Y-m');
	//文件夹不存在，先生成文件夹
	if(!file_exists($path)){
	    mkdir($path,0777,true);
	}
	$name = date('YmdHis').".jpg";
	$name2 = date('YmdHis')."_thumb.jpg";
	$fpname = $path.'/'.$name;
	$fpname2 = $path.'/'.$name2;
	$url='upload/'.$dir."/".date('Y-m').'/'.$name;
	$uploadPic = base64_decode($uploadPic);
	$a = file_put_contents($fpname, $uploadPic);
	$b = file_put_contents($fpname2, $uploadPic);
	if($a){
		//头像上传
		if($dir == 'users'){
			$userId = (int)session('WST_USER.userId');
			$rs = db('users')->where('userId',$userId)->update(['userPhoto'=>$url]);
			if($rs){
...
//购买服务类型
function WSTTypeName($type){
	if($type==1)return '果品购买';
	if($type==2)return '现场采摘';
	if($type==3)return '果树领养';
	if($type==4)return '果园众包';
	if($type==5)return '现采预定';
}