...
class Users extends Base{
    ...
    public function checkLogin(){
    	...
    	if(!empty($rs)){
    		...
    		}elseif (input('post.typ')==3) {
                $agencyrs = $this->where(["dataFlag"=>1, "userStatus"=>1,"userType"=>2,"userId"=>$userId])->find();
                if(empty($agencyrs)){
                    return WSTReturn('您不是区域商！');
                }
            }
            ...
            //如果是区域商则加载区域商信息
            if ($rs['userType']==2) {
                $agency = model('agencys')->where(["userId"=>$userId,"dataFlag" =>1])->find();
                if(!empty($agency))$rs = array_merge($agency->toArray(),$rs->toArray());
            }
    		//记录登录日志
    		$data = array();
    		$data["userId"] = $userId;
