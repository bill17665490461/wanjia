...
class Orders extends Base{
	...
	public function submit(){
		...
		try{
			...
			foreach ($carts['carts'] as $ckey =>$shopOrder){
				...
				$order['agencyId'] = Db::name('shops')->where('shopId='.$shopOrder['shopId'])->value('agencyId');
				//区域商比率
				if($order['agencyId'] != 0){
					$order['agencyRate'] = Db::name('agencys')->where('agencyId='.$order['agencyId'].' and agencyStatus=1 and dataFlag=1')->value('commissionRate');
				}
				...
				if(false !== $result){
					...
					foreach ($shopOrder['list'] as $gkey =>$goods){
						...
						$orderGoods['goodsType'] = $goods['type'];
	...
	public function userOrdersByPage($orderStatus,$isAppraise = -1){
		...
		$page = $this->alias('o')->join('__SHOPS__ s','o.shopId=s.shopId','left')
		             ...
		             ->field('o.orderId,o.orderNo,s.shopName,s.shopId,s.shopQQ,s.shopWangWang,o.goodsMoney,o.totalMoney,o.realTotalMoney,
		              o.orderStatus,o.deliverType,deliverMoney,isPay,payType,payFrom,o.orderStatus,needPay,isAppraise,isRefund,orderSrc,o.createTime,oc.complainId,orf.id refundId,orf.refundStatus')
	 ...
	 * 手机端获取商家订单分页
	 */
	public function ordersPages($orderStatus,$isAppraise = -1){
		$userId = (int)session('WST_USER.userId');
		$orderNo = input('post.orderNo');
		$shopName = input('post.shopName');
		$isRefund = (int)input('post.isRefund',-1);
		...
		$page = $this->alias('o')->join('__SHOPS__ s','o.shopId=s.shopId','left')
			         ...
			         ->order('o.createTime', 'desc')
			         ->paginate(10,false,['type'=>'BootstrapDetailed']);
		$pages = $page->render();
		return $pages;
	}
	/**
	...
	public function receive(){
		...
		$order = $this->alias('o')->join('__SHOPS__ s','o.shopId=s.shopId','left')
		              ...
		              ->field('o.orderId,o.orderNo,o.agencyId,o.goodsMoney,o.payType,s.userId,s.shopId,o.orderScore,o.agencyRate')->find();
		if(!empty($order)){
			Db::startTrans();
		    try{
		    	//计算订单佣金 增加商品销售量
				$goods = Db::name('order_goods')->field('goodsId,goodsNum,goodsPrice,commissionRate')->where('orderId',$order['orderId'])->select();
				$commissionFee = 0;
				foreach ($goods as $key => $v) {
					if((float)$v['commissionRate']>0){
						$commissionFee += round($v['goodsPrice']*$v['goodsNum']*$v['commissionRate']/100,2);
					}
					model('goods')->where('goodsId='.$v['goodsId'])->setInc('saleNum',$v['goodsNum']);
				}
				//计算区域商佣金
				$agencyFee = round($order['goodsMoney']*$order['agencyRate']/100,2);

				$data = ['orderStatus'=>2,'receiveTime'=>date('Y-m-d H:i:s'),'commissionFee'=>$commissionFee,'agencyFee'=>$agencyFee];
			    $result = $this->where('orderId',$order['orderId'])->update($data);
				if(false != $result){
					//修改商家未计算订单数
					...
					Db::execute($upSql);
					//修改区域商未结算订单数
					$upSql2 = 'update '.$prefix.'agencys set noSettledOrderNum=noSettledOrderNum+1,noSettledOrderFee=noSettledOrderFee+'.$agencyFee.' where agencyId='.$order['agencyId'];
					Db::execute($upSql2);
					//新增订单日志
					$logOrder = [];
					$logOrder['orderId'] = $orderId;
	...
	*/
	...
	public function wxOrder(){
		...
		$data = ['orderStatus'=>2,'receiveTime'=>date('Y-m-d H:i:s'),'isPay'=>1];//,'commissionFee'=>$commissionFee,'agencyFee'=>$agencyFee
