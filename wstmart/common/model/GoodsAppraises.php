...
class GoodsAppraises extends Base{
	...
	* 用户评价（手机端）
	*/
	public function userAppraiseMob(){
		$userId = (int)session('WST_USER.userId');
		...
		$data = $model->alias('g')
					  ...
					  ->each(function($item, $key){
					  		if($item['images']){
					  			$item['image'] = explode(',', $item['images']);						    	
					  		}else{
					  			$item['image'] = '';
					  		}
					  		return $item;					    
	...
	*/
	public function getById(){
		...
		$rs  = 	$this->alias('ga')
					 ...
					 ->join('__ORDER_GOODS__ og','og.orderId=ga.orderId and og.goodsId=ga.goodsId and og.goodsSpecId=ga.goodsSpecId','inner')
	...
	*/
	public function getByIdMob(){
		// 处理匿名
		// $anonymous = (int)input('anonymous')?(int)input('anonymous'):1;
		$goodsId = (int)input('goodsId');
		$where = ['ga.goodsId'=>$goodsId,
				  'ga.dataFlag'=>1,
				  'ga.isShow'=>1];
		...
		foreach ($rs as $k=>$v) {
			// 格式化时间
		 	...
		 	// 总评分
		 	$v->avgScore = ceil(($v->goodsScore + $v->serviceScore + $v->timeScore)/3);
			$start = floor((strlen($v->loginName)/2))-1;
			$v->loginName = substr_replace($v->loginName,'**',$start,2);						
			//获取用户等级
			$rrs = Db::name('user_ranks')->where('startScore','<=',$v->userTotalScore)->where('endScore','>=',$v->userTotalScore)->where('dataFlag=1')->field('rankId,rankName,userrankImg')->find();
			$v->userTotalScore = $rrs['userrankImg'];
			$v->rankName = empty($rrs['rankName'])?' ':$rrs['rankName'];
			//评论图片
			if($v->images){
				$v->image = explode(',',$v->images);
			}
		}				
		return $rs;
	}
