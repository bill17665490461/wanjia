...
class Favorites extends Base{
	...
	public function getShopsMob(){
		...
		->where(['f.userId'=> $userId,'favoriteType'=> 1])
		->order('f.favoriteId desc')
		->paginate(6,false,['type'=>'BootstrapDetailed'])->each(function($item,$key){
			$goods = db('goods')->where(['dataFlag'=> 1,'isSale'=>1,'shopId'=> $item["shopId"]])->field('goodsId,goodsName,shopPrice,goodsImg')
			->limit(4)->order('saleTime desc')->select();			
			$item['goods'] = $goods;
			return $item;
		});
		return $page;
