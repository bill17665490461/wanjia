...
class Carts extends Base{
	...
	public function addCart(){
		...
		$goods = $this->where(['userId'=>$userId,'goodsId'=>$goodsId,'type'=>$type,'goodsSpecId'=>$goodsSpecId])->select();
		if(empty($goods)){
			$data = array();
			$data['userId'] = $userId;
			...
			$rs = $this->where(['userId'=>$userId,'goodsId'=>$goodsId,'type'=>$type,'goodsSpecId'=>$goodsSpecId])->setInc('cartNum',$cartNum);
		    if(false !==$rs){
				return WSTReturn("添加成功", 1);
			}
		}
	 ...
	 * 取消购物车商品选中状态
	 */
	public function disChkGoods($goodsId,$type,$goodsSpecId,$userId){
		$this->save(['isCheck'=>0],['userId'=>$userId,'goodsId'=>$goodsId,'type'=>$type,'goodsSpecId'=>$goodsSpecId]);
	}
	...
	public function getCarts($isSettlement = false){
		...
		$rs = $this->alias('c')->join('__GOODS__ g','c.goodsId=g.goodsId','inner')
		           ...
		           ->where($where)
		           ->field('c.goodsSpecId,c.cartId,c.type,s.userId,s.shopId,s.shopName,g.goodsId,s.shopQQ,shopWangWang,g.goodsName,g.shopPrice,g.pickPrice,g.adoptPrice,g.crowdPrice,g.reservePrice,g.goodsStock,g.isSpec,gs.specPrice,gs.specStock,g.goodsImg,c.isCheck,gs.specIds,c.cartNum,g.goodsCatId')
		           ->select();
		$carts = [];
		$goodsIds = [];
		$goodsTotalNum = 0;
		...
		foreach ($rs as $key =>$v){
			...
			if($v['isSpec']==1 && $v['type']==1){
				$v['shopPrice'] = $v['specPrice'];
				$v['goodsStock'] = $v['specStock'];
			}else if($v['type']==2){
				$v['shopPrice'] = $v['pickPrice'];
			}else if($v['type']==3){
				$v['shopPrice'] = $v['adoptPrice'];
			}else if($v['type']==4){
				$v['shopPrice'] = $v['crowdPrice'];
			}else if($v['type']==5){
				$v['shopPrice'] = $v['reservePrice'];
			}
			...
			if($isSettlement && $v['allowBuy']!=10){
				$this->disChkGoods($v['goodsId'],(int)$v['type'],(int)$v['goodsSpecId'],(int)session('WST_USER.userId'));
				continue;
			}
			if($v['isCheck']==1){
	...
	public function getCartInfo($isSettlement = false){
		...
		$rs = $this->alias('c')->join('__GOODS__ g','c.goodsId=g.goodsId','inner')
		           ->join('__GOODS_SPECS__ gs','c.goodsSpecId=gs.id','left')
		           ->where($where)
		           ->field('c.goodsSpecId,c.cartId,c.type,g.goodsId,g.goodsName,g.shopPrice,g.pickPrice,g.adoptPrice,g.crowdPrice,g.reservePrice,g.goodsStock,g.isSpec,gs.specPrice,gs.specStock,g.goodsImg,c.isCheck,gs.specIds,c.cartNum')
		           ->select();
		$goodsIds = []; 
		$goodsTotalMoney = 0;
		$goodsTotalNum = 0;
