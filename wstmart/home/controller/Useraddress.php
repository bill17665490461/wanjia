...
class Useraddress extends Base{
    ...
    */
	...
	public function index(){
        if($this->isMobile()){
            ...
            $data = model('Home/userAddress')->listQuery($userId);
	...
	*/
	public function edit(){
        ...
        if($this->isMobile()){
            if(!empty($data)){
                $area = $data['areaIdPath'];
                $data['areaIdPath'] = str_replace('_',',',$data['areaIdPath']);
                $data['areaIdPath']=rtrim($data['areaIdPath'], ",");
                $area = explode("_",$area);
                $sheng = db('areas')->where('areaId',$area[0])->value('areaName');
                $shi = db('areas')->where('areaId',$area[1])->value('areaName');
                $qu = db('areas')->where('areaId',$area[2])->value('areaName');
                $area = $sheng.' '.$shi.' '.$qu;
