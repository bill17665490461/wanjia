...
class Users extends Base{
    ...
    public function index(){        
        ...
        $m = new MUsers();
        $userId = (int)session('WST_USER.userId');
        $user_data = $m->getById($userId);
        $this->assign('user_data',$user_data);

        $fa_goods = db('favorites')->where('userId='.$userId.' and favoriteType=0')->count();
        $fa_shops = db('favorites')->where('userId='.$userId.' and favoriteType=1')->count();
        $this->assign(['fa_goods'=>$fa_goods,'fa_shops'=>$fa_shops]);
        //浏览记录数量（手机）
        $ids = cookie("history_goods");
        $this->assign('historyNum',count($ids));
    ...
    */
    ...
    public function changeUserName(){
        ...
        $userId = (int)session('WST_USER.userId');
        $rs = model('users')->where('userId='.$userId)->update(['userName'=>$userName]);
        if($rs !== false){
            return WSTReturn('',1,$rs);
        }else{
            return WSTReturn('',-1,$rs);
    ...
    //手机端修改真实姓名trueName
    public function changeTrueName(){
        $trueName = input('post.trueName');
        $userId = (int)session('WST_USER.userId');
        $rs = model('users')->where('userId='.$userId)->update(['trueName'=>$trueName]);
        if($rs !== false){
            return WSTReturn('',1,$rs);
        }else{
            return WSTReturn('',-1,$rs);
    ...
    //手机端修改性别userSex
    public function changeUserSex(){
        $userSex = input('post.userSex');
        $userId = (int)session('WST_USER.userId');
        $rs = model('users')->where('userId='.$userId)->update(['userSex'=>$userSex]);
        if($rs !== false){
            return WSTReturn('',1,$rs);
        }else{
            return WSTReturn('',-1,$rs);
    ...
    public function getBrithday(){
        ...
        return $this->fetch('users/getBrithday');
    }
    //手机端修改生日brithday
    public function changeBrithday(){
        $brithday = input('post.brithday');
        $userId = (int)session('WST_USER.userId');
        $rs = model('users')->where('userId='.$userId)->update(['brithday'=>$brithday]);
        if($rs !== false){
            return WSTReturn('',1,$rs);
        }else{
            return WSTReturn('',-1,$rs);
    ...
    //手机端修改QQ userQQ
    public function changeUserQQ(){
        $userQQ = input('post.userQQ');
        $userId = (int)session('WST_USER.userId');
        $rs = model('users')->where('userId='.$userId)->update(['userQQ'=>$userQQ]);
        if($rs !== false){
            return WSTReturn('',1,$rs);
        }else{
            return WSTReturn('',-1,$rs);
    ...
    public function getChangePhoto(){
        return $this->fetch('users/getChangePhoto');
    }
}
