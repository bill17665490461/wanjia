...
use wstmart\common\model\GoodsAppraises as GA;
/**
 * ============================================================================
 *  MKT多用户商城
...
class Goods extends Base{
    ...
    */
    ...
    public function edit(){
        ...
        $object['goodsDesc']=$this->changeImgSrc($object['goodsDesc']);
        $object['pickDesc']=$this->changeImgSrc($object['pickDesc']);
        $object['adoptDesc']=$this->changeImgSrc($object['adoptDesc']);
        $object['crowdDesc']=$this->changeImgSrc($object['crowdDesc']);
        $object['reserveDesc']=$this->changeImgSrc($object['reserveDesc']);
    ...
    public function search(){
        ...
        if($data['areaId']!==0){
           ...
           // echo $selectArea['areaName'].'111111111111111';
    ...
    public function lists(){
	 ...
	 $this->assign('Tabarea3',$data['area1']);
	 $this->assign('Tabarea2',$data['area2']);
	 $this->assign('Tabarea1',$data['area3']);
	 
	 $this->assign('TabareaId',$data['areaId']);
	...
	$arr = array();
	if(!empty($data['goodsPage']['Rows'])){
		foreach($data['goodsPage']['Rows'] as $key=>$val){
		
			$arr[] = $val['goodsId'];
		}
	}
	
	$str = model('Goods')->getGoodsAddId($arr);
	$getShopsAddId = model('Shops')->getShopsAddId($str);
	
	
	$this->assign('getShopsAddId',$getShopsAddId);
	//dump($getShopsAddId);
    ...
    public function detail(){
        ...
        $GA= new GA();
        $apprNum = $GA->getGoodsEachApprNum(input('id'));
        $this->assign('apprNum',$apprNum);
    	...
    	if(!empty($goods)){
			...
			if(!empty($history)){
             ...
             // 商品详情延迟加载
            $goods['goodsDesc']=$this->changeImgSrc($goods['goodsDesc']);
            $goods['pickDesc']=$this->changeImgSrc($goods['pickDesc']);
            $goods['adoptDesc']=$this->changeImgSrc($goods['adoptDesc']);
            $goods['crowdDesc']=$this->changeImgSrc($goods['crowdDesc']);
            $goods['reserveDesc']=$this->changeImgSrc($goods['reserveDesc']);
            $goods['shop']['shopContent']=$this->changeImgSrc($goods['shop']['shopContent']);
    ...
    //手机端底部弹出内容
    public function btmLayer(){
        $m = new M();
        $goods = $m->getBySale(input('id/d',0));
        $this->assign('goods',$goods);
        return $this->fetch('goods_detail_bottom');
    }
    /**
     * 预警库存
     */
     ...
     * 手机端获取商品浏览记录
     */
    public function historyByGoodsMob(){
        $rs = model('Tags')->historyByGoodsMob();
        $this->assign('data',$rs);
        return $this->fetch('users/history/list_goods');
    }
}
