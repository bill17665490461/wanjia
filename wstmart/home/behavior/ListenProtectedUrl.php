...
class ListenProtectedUrl 
{
    public function run(&$params){
        ...
        if(isset($urls[$visit])){
			...
			if((int)session('WST_USER.agencyId')>0 && (int)session('WST_USER.shopId')==0)$userType = 2;
            // echo $userType;exit;
        	//未登录不允许访问受保护的资源
        	if($userType==-1){
		        if($request->isAjax()){
        	...
        	if(($userType==0 || $userType==2) && $menuType==1){
        	    if($request->isAjax()){
            ...
            //已登录但不是区域商 则不允许访问受保护的区域商资源
            if(($userType==0 || $userType==1) && $menuType==2){
                if($request->isAjax()){
                    echo json_encode(['status'=>-999,'msg'=>'对不起，您不是区域商，不能访问']);
                }else{
                    header("Location:".url('home/agencys/login'));
                }
                exit();
            }
