...
class Goods extends CGoods{
	 ...
	 */
     public function getBySale($goodsId){
		...
		if(!empty($rs)){
			...
			//手机端获取默认销售规格
			if($this->isMobile()){
				$defaultSpec = Db::name('goods_specs')->where('goodsId='.$goodsId.' and isDefault=1')->find();
				$defaultSpec['specIds'] = explode(':',$defaultSpec['specIds']);
				$rs['defaultSpec']=$defaultSpec;
			}			
			//获取商品属性
	...
	*/
	...
	public function pageQuery($goodsCatIds = []){
		...
		$gostatus = Input('gostatus/d');//服务ID
		if($gostatus==2){
			$where['g.pickPrice'] = ['>',0];
		}elseif($gostatus==3){
			$where['g.adoptPrice'] = ['>',0];
		}elseif($gostatus==4){
			$where['g.crowdPrice'] = ['>',0];
		}elseif($gostatus==5){
			$where['g.reservePrice'] = ['>',0];
		}
		//print_r($where);die;
		...
		if($this->isMobile()){
			$list = Db::name("goods")->alias('g')->join("__SHOPS__ s","g.shopId = s.shopId")
				...
				->paginate(14,false,['type'=>'BootstrapDetailed']);
		}else{
			$list = Db::name("goods")->alias('g')->join("__SHOPS__ s","g.shopId = s.shopId")
				->where($where)
				->field('goodsId,goodsName,goodsSn,goodsStock,saleNum,shopPrice,marketPrice,isSpec,goodsImg,appraiseNum,visitNum,s.shopId,shopName')
	...
	public function editGoodsBase(){
		...
		$rs = $this->update($data,['goodsId'=>$goodsId,'shopId'=>$shopId]);
		if($rs!==false){
	...
	* 获取商家Id
	*/
	public function getGoodsAddId($str){
		$shopsAddId = array();
		foreach($str as $k=>$v){
			$shopsAddId[] = Db::name("goods")->where(['goodsId'=>$v])->field('shopId')->find();
		}
		return $shopsAddId;
