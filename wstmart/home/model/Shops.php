...
class Shops extends CShops{
    ...
    public function getPages($pagesize){
        ...
        ->order('s.shopId asc')
        ->field('s.shopId,s.shopImg,s.shopName,s.shopTel,s.shopQQ,s.shopWangWang,s.shopCompany,ss.totalScore,ss.totalUsers,ss.goodsScore,ss.goodsUsers,ss.serviceScore,ss.serviceUsers,ss.timeScore,ss.timeUsers,.u.loginName,f.favoriteId,s.areaIdPath')
        ->paginate($pagesize,false,['type'=>'BootstrapDetailed']);
        $pages = $page->render();
        return $pages;
    }
    /**
     ...
     * 获取区域商店铺
     */ 
    public function agencyShops($agencyId){
        $data = $this->field('shopId,shopSn,shopName,shopKeeper,shopImg,telephone,shopAddress,shopStatus')->where(['agencyId'=>$agencyId,'dataFlag'=>1])->paginate()->toArray();
        if($data !== false){
    ...
    */
    public function getContent($id){
        $shopContent = db('shops')->where('shopId',$id)->value('shopContent');
        return $shopContent;
    }
    //保存果园介绍
    public function contentSave($id){
        $data = $data = input('post.');
        $rs = $this->where('shopId',$id)->update($data);
        if(false!==$rs){
    ...
    public function getAreaShops(){
        $areaIdPaths = db('shops')->field('shopName,areaIdPath,shopId,shopImg,shopContent')->select();
        //有果园入驻的省份ID
        $areaShops = [];
        $shengNames = [];
        foreach ($areaIdPaths as $key => $v) {
            $v['shopContent'] = $this->changeImgSrc($v['shopContent']);
            $areaId_arr = explode('_', $v['areaIdPath']);
            
            $sheng = db('areas')->where('areaId',$areaId_arr[0])->value('areaName');
           /* $shi = db('areas')->where('areaId',$areaId_arr[1])->value('areaName');
            $xian = db('areas')->where('areaId',$areaId_arr[2])->value('areaName');
            if(!in_array($sheng,$shengNames)){
                $shengNames[$areaId_arr[0]] = $sheng;
            }
            $v['areaName1'] = $shi;
            $v['areaName2'] = $xian;
            $areaShops[$areaId_arr[0]][] = $v;  */        
        }
        return array($areaShops,$shengNames);
    }
	...
	* 获取商家地址
	*/
	public function getShopsAddId($str){
		$shops = array();
		$shopsAddId = array();
		foreach($str as $k=>$v){
			$shops = db('shops')->where('shopId',$v['shopId'])->field('areaId')->find();
			
			//echo $shops[$b];
			$shopsAddId[] = db('areas')->where('areaId',$shops['areaId'])->field('areaId,parentId')->find();
			
			if($shopsAddId[$k]['parentId'] > 0){
			
				$shopsAddId[$k]['id'] = db('areas')->where('areaId',$shopsAddId[$k]['parentId'])->field('areaId,parentId')->find();
			}
     ...
     * 判断
     */
    public function getIsShopName($txt){
        if (empty($txt)){
            return false;
        }
       $data = $this->where(['shopName'=>$txt,'dataFlag'=>1])->field('shopStatus')->find();
        return $data;
    }
}
