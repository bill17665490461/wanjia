$(function(){
	WST.slides();
	$(".sheng-item").eq(0).addClass('j-sheng-selected');
	$('.shengShops').eq(0).show();
});
WST.slides = function(){
...
function gpanelOver2(obj){
	var sid = $(obj).attr("id");	
	$(obj).siblings().removeClass("j-sheng-selected");	
	$("#"+sid).addClass("j-sheng-selected");	
	$(".shengShops").hide();
	$("#"+sid+"_pl").show();
}
/*楼层*/
...
function shopContent(id){
	var html = $("#shopContent"+id);
	layer.open({
	  	type: 1,
	  	title: '果园介绍',
	  	...
	  	shadeClose: true,
    	shade: 0.8,
	  	area: ['750px', ($(window).height() - 50) +'px'], //宽高
	  	content: html
	});
}