...
function view(val){
    location.href=WST.U('home/agencySettlements/view','id='+val);
}
function getQueryPage(p){
	...
	$.post(WST.U('home/agencysettlements/pageQueryAgency'),params,function(data,textStatus){
...
function settlement(){
	...
	WST.confirm({content:'您确定要申请结算这些订单吗？',yes:function(){
        $.post(WST.U('home/agencysettlements/agencySettlement'),{ids:ids.join(',')},function(data,textStatus){
			layer.close(load);
			var json = WST.toJson(data);
		    if(json.status==1){
...
function getUnSettledOrderPage(p){
	...
	$.post(WST.U('home/agencysettlements/agencyUnSettledQuery'),params,function(data,textStatus){
		var json = WST.toJson(data);
	    if(json.status==1){
...
function getSettleOrderPage(p){
	...
	$.post(WST.U('home/agencysettlements/agencySettledQuery'),params,function(data,textStatus){
		var json = WST.toJson(data);
	    if(json.status==1){
