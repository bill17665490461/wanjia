...
function WSTShopCats($parentId){
	...
	$dbo = Db::table('_WSTReutrn_SHOP_CATS__')->where(['dataFlag'=>1, 'isShow' => 1,'parentId'=>$parentId,'shopId'=>$shopId]);
 ...
 * 列表
 */
function lWSTSideAccreds(){
    return Db::name('accreds')->where('dataFlag',1)->field(true)->select();
}
/**
 * 获取首页左侧分类、推荐品牌和广告
 */
